...
void main() {
  ...
  group('ffigen Phase A', () {
    ...
    test('libsmb2 6.2 context introspection symbols resolve', () {
      final ctx = bindings.smb2_init_context();
      expect(ctx, isNot(equals(nullptr)));
      try {
        // These calls are local and do not initiate network I/O. Calling
        // them forces ffigen's generated lookups for the symbols added in
        // the libsmb2-6.2 public ABI.
        bindings.smb2_get_user(ctx);
        bindings.smb2_get_domain(ctx);
        expect(bindings.smb2_delegate_credentials(ctx, ctx), -1);
      } finally {
        bindings.smb2_destroy_context(ctx);
      }
    });
